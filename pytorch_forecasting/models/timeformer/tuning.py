...
from pytorch_forecasting import Timeformer
...
def optimize_hyperparameters(
    ...
    n_embd_range: Tuple[int,int] = [16,128],
    max_cat_embd_range: Tuple[int,int] = [8,16],
    n_real_embd_range: Tuple[int,int] = [4,8],
    n_head_range: Tuple[int,int] = [1,4],
    n_cross_range: Tuple[int,int] = [1,6],
    ...
    **kwargs,
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        # Suggest n_head from the n_head_range
        n_head = trial.suggest_int('n_head', *n_head_range)
        # Suggest a factor such that the product of n_head and factor is within n_embd_range
        min_factor = max(math.ceil(n_embd_range[0] / n_head), 1)
        max_factor = n_embd_range[1] // n_head
        factor = trial.suggest_int("factor", min_factor, max_factor)
        # Calculate n_embd as the product of n_head and factor
        n_embd = n_head * factor
        
        config = {'n_embd': n_embd,
        'max_cat_embd': trial.suggest_int("max_cat_embd", *max_cat_embd_range),
        'n_real_embd': trial.suggest_int('n_real_embd', *n_real_embd_range),
        'n_head': n_head,
        'dropout': trial.suggest_float("dropout", *dropout_range),
        'bias': True,
        'n_cross': trial.suggest_int('n_cross', *n_cross_range)}
        
        model = Timeformer.from_dataset(
        ...
        # model = Timeformer.load_from_checkpoint(path)  # make better
        # # Evaluate the best model on validation dataloader
        # trainer.test(model = model, dataloaders =val_dataloaders, verbose=False)
